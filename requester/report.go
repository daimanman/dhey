	...
	for res := range r.results {
		...
		if res.err != nil {
			...
			if len(r.resLats) < maxRes {
				...
				r.connLats = append(r.connLats, res.dnsDuration.Seconds())
...
func (r *report) finalize(total time.Duration) {
	r.total = total
	size := float64(len(r.lats))
	r.rps = float64(r.numRes) / r.total.Seconds()
	r.average = r.avgTotal / size
	r.avgConn = r.avgConn / size
	r.avgDelay = r.avgDelay / size
	r.avgDNS = r.avgDNS / size
	r.avgReq = r.avgReq / size
	r.avgRes = r.avgRes / size
	r.print()
}
...
func (r *report) snapshot() Report {
	...
	snapshot := Report{
		...
		ConnLats:       make([]float64, length),
		DnsLats:        make([]float64, length),
		ReqLats:        make([]float64, length),
		ResLats:        make([]float64, length),
		DelayLats:      make([]float64, length),
		TaskId:         r.taskId,
		Duration:       r.total.Seconds(),
	}
	...
	snapshot.SizeReq = r.sizeTotal / int64(length)
	...
	snapshot.ResMin = r.reqLats[len(r.resLats)-1]
...
func (r *report) latencies() []LatencyDistribution {
	...
	lenLats := len(r.lats)
	j := 0
	for i := 0; i < lenLats && j < lenPctls; i++ {
		current := i * 100 / lenLats
		if current >= pctls[j] {
			data[j] = r.lats[i]
			j++
		}
	...
	res := make([]LatencyDistribution, lenPctls)
	for i := 0; i < lenPctls; i++ {
		if data[i] > 0 {
...
func (r *report) histogram() []Bucket {
	...
	for i := 0; i < len(buckets); i++ {
		res[i] = Bucket{
			Mark:       buckets[i],
			Count:      counts[i],
			Frequentcy: float64(counts[i]) / float64(len(r.lats)),
		}
	}
	return res
}
